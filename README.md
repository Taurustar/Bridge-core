...
|---|---|---|
| Real-time server | FastAPI + WebSocket | FastAPI + WebSocket (same architecture) |
| Multi-device sync / heartbeats | ✅ | ✅ |
| Delivery guarantees | ✅ | ✅ startup reconciliation + client `message_ack` |
| Client modes | Companion + Work | Companion + Work |
| Clients served | Unity, Desktop, Android, Discord, WhatsApp | Generic WS clients; Unity client in progress |

## LLM Providers

| Provider | Akane | Bridge |
|---|---|---|
| Fireworks | ✅ primary | ✅ primary |
| Chutes | ✅ fallback | ✅ fallback |
| Kimi (Moonshot) | ✅ emergency | ❌ |
| Ollama (local) | ✅ | ✅ |
| OpenAI-compatible endpoint | ❌ | ✅ |
| Failover chain + function calling | ✅ | ✅ |
...
|---|---|---|
| Short-term / durable store | Redis | Redis |
| Semantic memory | ChromaDB default; **live pgvector backend** (asyncpg pool, cosine retrieve); Qdrant/mem0 export-only | Optional Chroma + deterministic token-overlap fallback |
| Embeddings | ✅ dedicated embedder (Fireworks / local sentence-transformers / OpenAI) powering pgvector | 🟡 via Chroma |
| Organization | ✅ memory cabinet — Redis desk + pgvector archive, tagged drawers (owner facts / her lore / episodic) | ✅ mid-term chapters + durable-fact extraction |
| Maintenance | ✅ cleanup, export, snapshot, migration tooling | 🟡 precise wipe/delete only |
...
| Intimacy layer | ✅ | 🚫 absent by design (test-enforced exclusion) |
...
| Capability | Akane | Bridge |
|---|---|---|
| Sessions / MCP proxy / agent loop / device daemon / pause protocol | ✅ all | ✅ all |
...
| Datastore | Redis + ChromaDB / pgvector (PostgreSQL) | Redis + optional Chroma |
...
services or network are needed. Deployment checks use the release tooling:
`scripts/validate_config.py core.env` before any (re)start and
`scripts/ws_smoke.py --host <tailscale-ip> --user <owner> --probe-foreign-user`
